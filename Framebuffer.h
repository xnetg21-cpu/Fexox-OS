...
/* =========================================================================
 * Низкоуровневые параметры буфера (для sys_get_framebuffer — см. Syscall.c).
 * В VGA-режиме fb_phys_addr()/fb_pitch()/fb_bpp() возвращают 0 —
 * вызывающий код (Syscall.c) в этом случае сам подставляет VGA text
 * fallback (0xB8000, 80x25).
 * ========================================================================= */
...
/* =========================================================================
 * Оконный менеджер (генерический)
 *
 * Единая реализация окон + кнопок панели задач, которой пользуются:
 *   - внутренние "встроенные" приложения ядра (сейчас — My Computer,
 *     см. Framebuffer.c: fxapp-иконка на рабочем столе);
 *   - системные вызовы SYS_WIN_CREATE/SYS_WIN_CLOSE/SYS_WIN_MOVE
 *     (см. Syscall.c) — то есть настоящие userspace-приложения.
 *
 * И то, и другое в итоге вызывает один и тот же код ниже — поэтому
 * "все приложения работают через один и тот же оконный механизм",
 * независимо от того, кто его вызвал: ядро напрямую или syscall.
 *
 * Каждое открытое окно получает рамку (заголовок + кнопка закрытия +
 * белое тело) и кнопку на панели задач (отдельно от кнопки Menu,
 * с названием окна; если название не помещается — обрезается и
 * добавляется "..."). Кнопка/окно исчезают вместе при закрытии.
 * ========================================================================= */
#define UI_WIN_TITLE_MAX   64
#define UI_WIN_INVALID     (-1)

/* Создать окно с заголовком title, размером w×h (тело + заголовок).
 * Позиция подбирается автоматически (по центру рабочей области, с
 * небольшим смещением для каждого следующего окна, чтобы не накладывались
 * друг на друга). owner_pid — pid процесса-владельца (-1 для окон,
 * открытых самим ядром, например My Computer).
 * Возвращает id окна (>=0) или UI_WIN_INVALID, если свободных слотов нет. */
int ui_win_create(const char *title, uint32_t w, uint32_t h, int32_t owner_pid);

/* Закрыть окно и убрать его кнопку с панели задач. */
int ui_win_close(int win_id);

/* Переместить окно (например, при перетаскивании за заголовок). */
int ui_win_move(int win_id, int32_t x, int32_t y);

/* Закрыть все окна, принадлежащие owner_pid (вызывать при завершении
 * процесса — см. sc_exit в Syscall.c), чтобы окна "мёртвых" процессов
 * не оставались висеть на экране. */
